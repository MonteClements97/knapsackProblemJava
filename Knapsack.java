...
public class Knapsack {
    private PriorityQueue<Item> sortedItems;
    /**
    ...
    private double oneMethodToRuleThemAll(Comparator comparator, boolean isIO){
        int knapCap = this.capacity;
        double totalValue = 0;
        this.sortedItems = new PriorityQueue<>(comparator);
        for(Item i: items){
            this.sortedItems.add(i);
        }
        ArrayList<Item> knapsack = new ArrayList<>();
        System.out.print(items.length);
        while(knapCap > 0 && this.sortedItems.size() > 0){
            Item currentItem = this.sortedItems.poll();
            if (knapCap - currentItem.getWeight() >= 0){
                totalValue += currentItem.getValue();
                currentItem.setFraction(currentItem.getWeight());
                knapCap -= currentItem.getWeight();
                ...
                totalValue += (currentItem.getValue() * ((double)knapCap/(double)currentItem.getWeight()));
                currentItem.setFraction(knapCap);
                return totalValue;
            }
        }
        return totalValue;
